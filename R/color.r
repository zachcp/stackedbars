...
#' Use ColorBrewer palettes for stackedbar fill colors
#'
...
#' Use Tableau discrete palettes for stackedbar fill colors
#'
...
sg_fill_tableau<- function(sg, palette="tableau20") {
  ...
  if (palette %in%  c("tableau20", "tableau10medium", "gray5",
    ...
    warning("'palette' value is not a valid Tableau discrete color scale, using stackedbar defaults")
  }
...
#' Use manual colors for stackedbar fill colors
#'
#' Specify a vector of colors (e.g. \code{c("red", "#00ff00", rgb(0,0,1))}) to
#' use for the color scale. Note that \code{stackedbar} sorts the categorical values
...
sg_fill_manual <- function(sg, values=NULL) {
  ...
  if (is.null(values)) {
    ...
    warning("color values not specified, using stackedbar defauls")
